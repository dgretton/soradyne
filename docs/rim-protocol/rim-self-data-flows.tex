...
\begin{tikzpicture}[
  ...
  node/.style={draw=rimblue, fill=rimlight, rounded corners=4pt, minimum width=2cm, minimum height=0.8cm, font=\small},
  flowbox/.style={draw=rimdark, fill=white, rounded corners=8pt, thick, inner sep=12pt},
]
  % Flow boundary
  \node[flowbox, minimum width=10cm, minimum height=5cm] (flow) {};
  \node[above=0pt of flow.north, font=\small\bfseries\color{rimdark}] {Self-Data Flow};

  % Streams
  \node[node] (src1) at (-3, 1) {Source A};
  \node[node] (src2) at (-3, -1) {Source B};
  \node[node, fill=dripcolor!15] (drip) at (0, 0) {Drip};
  \node[node, fill=jetcolor!15] (jet1) at (0, 1.5) {Jet};
  \node[node, fill=fittingcolor!15] (fit) at (0, -1.5) {Fitting};
  \node[node] (sink) at (3, 0) {Sink};

  % Arrows
  \draw[stream] (src1) -- (jet1);
  \draw[stream] (src2) -- (fit);
  \draw[jet] (jet1) -- (sink);
  \draw[drip] (drip) -- (sink);
...
It covers:
\begin{itemize}[leftmargin=2em]
  \item The conceptual model: flows, streams, roles, fittings, policies
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item What roles are needed to operate the flow
...
\subsection{What a Role Is}

A role is an abstract unit of responsibility within a flow. Roles are defined in the
flow's type definition. Parties (devices, processes, services) \emph{fill} roles at
runtime.

\begin{definition}[Role]
A role is a named set of responsibilities defined by a flow type. A single device can
fill many roles. Many devices can fill the same role. Roles are abstract; they do not
...
A memorization role reliably stores and recalls data that was written to streams, so
...
Fittings are abstract concepts in the current design. They do not need to be reified
as structs or objects in code yet---the parties running the code implement the fittings
described in the flow's policies. A fitting might be as simple as ``apply each edit
operation from the edit stream to a CRDT, making the result available on the drip
stream,'' or as complex as ``mix $n$ audio input streams into a single output stream,
applying gain normalization and latency compensation.''

In the future, complex policies may require fitting objects that can be delegated,
composed, and hot-swapped. For now, fittings are nouns in documentation and comments,
not in code.
...
\begin{tikzpicture}[
  ...
  % Sources
  \node[role] (srcA) at (-4, 1) {Source (device A)};
  \node[role] (srcB) at (-4, -1) {Source (device B)};

  % Edit streams
  \node[streambox] (editA) at (-1, 1) {edit stream A};
  \node[streambox] (editB) at (-1, -1) {edit stream B};

  % Fitting
  \node[fitting] (fit) at (2.5, 0) {CRDT merge};

  % Memorization
  \node[role] (mem) at (2.5, -2) {Memorization};

  % Drip
  \node[streambox, fill=dripcolor!15] (drip) at (6, 0) {drip: current state};

  % Sink
  \node[role] (sink) at (9, 0) {Sink (UI)};
  ...
  \draw[stream, color=fittingcolor] (fit) -- (drip);
  \draw[stream, dashed] (fit) -- (mem) node[midway, right, font=\scriptsize\color{gray}] {persist};
...
How do you find the parties filling roles for a flow? Discovery policies specify:
\begin{itemize}[leftmargin=2em]
  \item \textbf{Simple}: The configuration contains a static list of hosts
  \item \textbf{Referral}: A known host can provide addresses of others
  \item \textbf{Registry}: A static remote host maintains a directory
  \item \textbf{DHT}: Distributed hash table lookup
  \item \textbf{Broadcast}: Local network discovery (BLE, mDNS)
\end{itemize}

The policy also specifies what to do when discovery information is stale (retire the
flow? attempt re-discovery? fall back to a different method?).
...
An image has rich spatial structure. It can be treated as a block of binary data (and
often is---editing locally always constitutes complete replacement on save), but a
flow-aware image preserves the structure that makes it an image.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Transform support}: A $2 \times 2$ matrix and a pair of start/end
    vectors can express any rotation, scale, flip, and crop. Most transforms are better
    done on the receiving device, but for very large images, the query should support
    spatial subsetting.
\end{itemize}

\subsubsection{Streams}
\begin{itemize}[leftmargin=2em]
  \item \textbf{Drip} (\texttt{pixel\_state}): The current pixel values of the
    squashed consensus edit state. Queryable by region, resolution, and transform.
  \item \textbf{Edit streams} (per-party): Layer operations---full replacement, overlay
    with blend mode (normal, multiply, mask, etc.), transform-and-crop, filters. Each
    edit references the drip state it was based on (causal context).
  \item \textbf{Jet} (\texttt{viewport}): Per-party, what region of the image each
    participant is currently viewing. Optional.
\end{itemize}

\subsubsection{Key Policies}
\begin{itemize}[leftmargin=2em]
  \item Conflict resolution for concurrent pixel edits: last-write-wins per region,
    or layer stacking order defined by the type.
  \item Large images: memorization roles may store tiles, serving subsets on query.
\end{itemize}
...
A photo album is a self-data flow whose data is a combination of structured metadata
(order, annotations, crops) and references to binary image data (which may itself be
managed by image flows or stored as opaque blobs).

\subsubsection{Streams}
\begin{itemize}[leftmargin=2em]
  \item \textbf{Drip} (\texttt{album\_state}): The current structure of the album.
    Queryable for the ordered list of photos, each photo's metadata (crop, rotation,
    annotations, tags, faces). Backed by a ConvergentDocument.
  \item \textbf{Edit streams} (per-party): Add photo, remove photo, reorder, crop,
    rotate, annotate. These benefit from receipt confirmation.
  \item \textbf{Jets} (per-party): Who is currently viewing what, who is typing a
    comment where, cursor positions. Prompt, non-consensus.
\end{itemize}

\subsubsection{Roles}
\begin{itemize}[leftmargin=2em]
  \item Standard: data source (editor), data sink (viewer), memorization (stores album
    state and image blobs), fitting (applies edits to album CRDT).
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Real-time membership}: Pieces join and leave ensembles as they come
    ...
    ensemble membership with both additions and subtractions.
...
A parure simplifies the ensemble's routing problem. When the ensemble's topology tracker
sees that all members of a parure are present, it can treat them as a pre-solved cluster
and focus routing decisions on the inter-parure links.
...
\begin{itemize}[leftmargin=2em]
  \item A microcontroller with storage (e.g., an ESP32 with an SD card) that holds
    dissolution shards
  \item An actuator that receives commands from a flow but does not route traffic
  \item A simple bridge to a different device or protocol (e.g., a BLE-to-serial adapter)
\end{itemize}

An accessory may implement routing ``through'' itself (acting as a relay between two
other pieces) or may simply offer its services without awareness of the broader topology.
The key design constraint is that implementing an accessory should be straightforward:
a small interface, minimal dependencies, no requirement for the full Soradyne library
stack. This makes it feasible to build accessories on resource-constrained hardware.
...
\textbf{Fitting} &
  A transformation connecting streams to each other. Abstract concept; not yet a
  struct/object in code. The code implementing a fitting is the code that takes data
  from one stream and produces data for another. \\[6pt]
...
\textbf{Role} &
  ...
  Examples: data source, data sink, memorization, curator, aligner. \\[6pt]
...
\textbf{Memorization} &
  A role responsible for reliably storing and recalling stream data. Subject to
...
Flow type definitions & Not started & Design documents per this spec. \\
Real I/O through flows & Not started & Streams are in-memory only. \\
...
Device topology (capsules) & Not started & Capsule creation, piece authorization, retirement. \\
Ensemble tracking & Not started & Real-time membership, BLE advertisements, topology sync. \\
BLE transport & Not started & Baseline BLE stream implementation. \\
\bottomrule
\end{tabularx}
\end{center}
