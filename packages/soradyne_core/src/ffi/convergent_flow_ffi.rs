...
/// Connect a flow directly to a TCP peer for sync (no capsule/pairing needed).
///
/// `peer_addr_ptr`: "ip:port"
/// `listen`:  1 = bind and wait for incoming connection (server role)
...
pub extern "C" fn soradyne_flow_connect_tcp(
    ...
    let socket_addr: std::net::SocketAddr = match addr_str.parse() {
        Ok(a) => a,
        Err(_) => {
            eprintln!("[convergent_flow_tcp] invalid address: {}", addr_str);
            return -3;
        }
    };
    ...
    let flow_arc = unsafe {
        let arc = Arc::from_raw(handle as *const Mutex<ConvergentFlow>);
    ...
    let result = runtime_handle.block_on(async move {
        ...
        let peer_id = if peer_bytes.len() == 16 {
            ...
            eprintln!(
                "[convergent_flow_tcp] bad handshake: expected 16 bytes, got {}",
                peer_bytes.len()
            );
            return Err(crate::ble::BleError::ConnectionError(
        ...
        // 4. Wire flow to messenger and start sync
        {
            use crate::flow::flow_core::Flow;
            let mut flow = flow_arc.lock().map_err(|_| {
                crate::ble::BleError::ConnectionError("flow lock poisoned".into())
            })?;
            with_drip_flow_mut!(flow, f => {
                f.set_ensemble(Arc::clone(&messenger), Arc::clone(&topology));
                f.start().map_err(|e| {
        ...
        eprintln!(
            "[convergent_flow_tcp] connected: local={} peer={}",
            local_uuid, peer_id
        );
        ...
        // 6. Horizon exchange
        {
            ...
            let (flow_id, local_horizon) = {
                ...
                with_drip_flow!(flow, f => {
                    let id = f.id();
                    let doc = f.document().read().map_err(|_| {
                        crate::ble::BleError::ConnectionError("doc lock".into())
                    })?;
                    let horizon = doc.horizon().clone();
                    Ok::<_, crate::ble::BleError>((id, horizon))
                })?
            };
    ...
    match result {
        Ok(()) => 0,
        Err(e) => {
            eprintln!("[convergent_flow_tcp] connect failed: {}", e);
            -10
        }
    }
}
